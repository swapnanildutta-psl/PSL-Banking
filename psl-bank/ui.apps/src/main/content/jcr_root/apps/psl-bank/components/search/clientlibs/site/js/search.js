...
(function () {
  ...
  function updateSearchResultsStatusMessageElement(
    ...
    var searchResultsStatusMessage = document.querySelector(
      "#" + searchElementId + "> .cmp_search__info"
    );
    searchResultsStatusMessage.style.visibility = "visible";
    var searchResultsFoundMessage =
      totalResults === 1 ? totalResults + " result" : totalResults + " results";
    var searchResultsNotFoundMessage = "No results";
    searchResultsStatusMessage.innerText =
      totalResults > 0
        ? searchResultsFoundMessage
    ...
    if (self._hasMoreResults) {
      ...
      request.onload = function () {
        ...
        if (request.status >= 200 && request.status < 400) {
          ...
          updateSearchResultsStatusMessageElement(
            self._elements.self.id,
            data.length
          );
          if (data.length > 0) {
  ...
  Search.prototype._hideSearchResultsStatusMessage = function () {
    var searchResultsStatusMessage = document.querySelector(
      "#" + this._elements.self.id + "> .cmp_search__info"
    );
